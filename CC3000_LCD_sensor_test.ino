...
#define WLAN_SSID       "Home"           // cannot be longer than 32 characters!
#define WLAN_PASS       "0933419796"
...
void setup(void)
{
  ...
  lcd.println(F("Initializing net..."));
  if (!cc3000.begin())
  {
  ...
  ip = 0xC0A86403;      /*"www.ab126.com/system/2859.html"*/
  lcd.setCursor(0, 0);
  lcd.println(F("Connecting server"));
...
void sensor(){
  ...
  soilValue = map(soilValue,250,1023,100,0); //將數值轉換為0%~100%表示
    ...
    if(isnan(h) || isnan(t)){
      ...
      lcd.print(soilValue);
      lcd.println("%        ");
      sensorValue = String("/update?key=ALSKUJG6DR5PLODE&field1=")+h+("&field2=")+t+("&field3")+soilValue;
